...
void LibcameraApp::ConfigureViewfinder()
{
	...
	configureDenoise(options_->denoise == "auto" ? "off" : options_->denoise);
	setupCapture();
...
void LibcameraApp::StartCamera()
{
	...
	{
		if (StillStream())
			controls_.set(controls::FrameDurationLimits, { INT64_C(100), INT64_C(10000000000) });
		else if (options_->framerate > 0)
		{
			int64_t frame_time = 1000000 / options_->framerate; // in us
			controls_.set(controls::FrameDurationLimits, { frame_time,  INT64_C(10000000000) }); //frame_time
		}
	}
